...
import com.qingkouwei.handyinstruction.common.interfaceOrImplement.DemoEmCallBack;
    ...
    public LiveData<Resource<String>> registerToHx(String userName, String pwd) {
        return new NetworkOnlyResource<String>() {
            ...
            protected void createCall(@NonNull ResultCallBack<LiveData<String>> callBack) {
                ...
                if(!com.qingkouwei.handyinstruction.DemoHelper.getInstance().isSDKInit) {
                    com.qingkouwei.handyinstruction.DemoHelper.getInstance().init(DemoApplication.getInstance());
                    com.qingkouwei.handyinstruction.DemoHelper.getInstance().getModel().setCurrentUserName(userName);
                }
                runOnIOThread(() -> {
                    try {
    ...
    public LiveData<Resource<EaseUser>> loginToServer(String userName, String pwd, boolean isTokenFlag) {
        return new NetworkOnlyResource<EaseUser>() {
            ...
            protected void createCall(@NonNull ResultCallBack<LiveData<EaseUser>> callBack) {
                com.qingkouwei.handyinstruction.DemoHelper.getInstance().init(DemoApplication.getInstance());
                com.qingkouwei.handyinstruction.DemoHelper.getInstance().getModel().setCurrentUserName(userName);
                com.qingkouwei.handyinstruction.DemoHelper.getInstance().getModel().setCurrentUserPwd(pwd);
                if(isTokenFlag) {
        ...
        return new NetworkOnlyResource<Boolean>() {
            ...
            protected void createCall(@NonNull ResultCallBack<LiveData<Boolean>> callBack) {
                EMClient.getInstance().logout(unbindDeviceToken, new EMCallBack() {
                    ...
                    @Override
                    public void onSuccess() {
                        com.qingkouwei.handyinstruction.DemoHelper.getInstance().logoutSuccess();
                        //reset();
                        if (callBack != null) {
                            callBack.onSuccess(createLiveData(true));
                        }
