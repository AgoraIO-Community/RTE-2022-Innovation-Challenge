package com.qingkouwei.handyinstruction.section.chat;
...
import com.qingkouwei.handyinstruction.MainActivity;
...
import com.qingkouwei.handyinstruction.common.interfaceOrImplement.ResultCallBack;
...
public class ChatPresenter extends EaseChatPresenter {
    ...
    private ChatPresenter() {
        ...
        //添加网络连接状态监听
        com.qingkouwei.handyinstruction.DemoHelper.getInstance().getEMClient().addConnectionListener(connectionListener);
        //添加多端登录监听
        com.qingkouwei.handyinstruction.DemoHelper.getInstance().getEMClient().addMultiDeviceListener(multiDeviceListener);
        //添加群组监听
        com.qingkouwei.handyinstruction.DemoHelper.getInstance().getGroupManager().addGroupChangeListener(groupListener);
        //添加联系人监听
        com.qingkouwei.handyinstruction.DemoHelper.getInstance().getContactManager().setContactListener(contactListener);
        //添加聊天室监听
        com.qingkouwei.handyinstruction.DemoHelper.getInstance().getChatroomManager().addChatRoomChangeListener(chatRoomListener);
        //添加对会话的监听（监听已读回执）
        com.qingkouwei.handyinstruction.DemoHelper.getInstance().getChatManager().addConversationListener(conversationListener);
    }
    ...
    public void clear() {
        if(connectionListener != null) {
            com.qingkouwei.handyinstruction.DemoHelper.getInstance().getEMClient().removeConnectionListener(connectionListener);
        }
        if(multiDeviceListener != null) {
            com.qingkouwei.handyinstruction.DemoHelper.getInstance().getEMClient().removeMultiDeviceListener(multiDeviceListener);
        }
        if(groupListener != null) {
            com.qingkouwei.handyinstruction.DemoHelper.getInstance().getGroupManager().removeGroupChangeListener(groupListener);
        }
        if(contactListener != null) {
            com.qingkouwei.handyinstruction.DemoHelper.getInstance().getContactManager().removeContactListener(contactListener);
        }
        if(chatRoomListener != null) {
            com.qingkouwei.handyinstruction.DemoHelper.getInstance().getChatroomManager().removeChatRoomListener(chatRoomListener);
        }
        if(conversationListener != null) {
            com.qingkouwei.handyinstruction.DemoHelper.getInstance().getChatManager().removeConversationListener(conversationListener);
        }
        if(handler != null) {
    ...
    public void onMessageReceived(List<EMMessage> messages) {
        ...
        for (EMMessage message : messages) {
            ...
            List<String>
                disabledIds = com.qingkouwei.handyinstruction.DemoHelper.getInstance().getPushManager().getNoPushGroups();
    ...
    private class ChatConnectionListener implements EMConnectionListener {
        ...
        public void onConnected() {
            ...
            if(!com.qingkouwei.handyinstruction.DemoHelper.getInstance().isLoggedIn()) {
                return;
            }
        ...
        public void onInvitationReceived(
            ...
            if(allMessages != null && !allMessages.isEmpty()) {
                for (EMMessage message : allMessages) {
                    ...
                    if(ext != null && (ext.containsKey(DemoConstant.SYSTEM_MESSAGE_GROUP_ID) && TextUtils
                        .equals(groupId, (String)ext.get(DemoConstant.SYSTEM_MESSAGE_GROUP_ID)))
    ...
    private class ChatContactListener implements EMContactListener {
        ...
        public void onContactAdded(String username) {
            ...
            EMClient.getInstance().userInfoManager().fetchUserInfoByUserId(userId, new EMValueCallBack<Map<String, EMUserInfo>>() {
                ...
                public void onSuccess(Map<String, EMUserInfo> value) {
                    ...
                    com.qingkouwei.handyinstruction.DemoHelper.getInstance().getModel().insert(entity);
                    com.qingkouwei.handyinstruction.DemoHelper.getInstance().updateContactList();
        ...
        public void onContactDeleted(String username) {
            ...
            boolean deleteUsername = com.qingkouwei.handyinstruction.DemoHelper.getInstance().getModel().isDeleteUsername(username);
            int num = com.qingkouwei.handyinstruction.DemoHelper.getInstance().deleteContact(username);
            com.qingkouwei.handyinstruction.DemoHelper.getInstance().updateContactList();
        ...
        public void onContactEvent(int event, String target, String ext) {
            ...
            switch (event) {
                case CONTACT_REMOVE: //好友已经在其他机子上被移除
                    ...
                    com.qingkouwei.handyinstruction.DemoHelper.getInstance().getChatManager().deleteConversation(target, false);
     ...
     * @param target1
     */
    private void removeTargetSystemMessage(
        String target1, String params1, String target2, String params2) {
        EMConversation conversation = EaseSystemMsgManager.getInstance().getConversation();
        List<EMMessage> messages = conversation.getAllMessages();
        if(messages != null && !messages.isEmpty()) {
            for (EMMessage message : messages) {
    ...
    private void saveGroupNotification(
        String groupId, String groupName, String inviter, String reason, InviteMessageStatus status) {
        Map<String, Object> ext = EaseSystemMsgManager.getInstance().createMsgExt();
        ext.put(DemoConstant.SYSTEM_MESSAGE_FROM, groupId);
        ext.put(DemoConstant.SYSTEM_MESSAGE_GROUP_ID, groupId);
        ext.put(DemoConstant.SYSTEM_MESSAGE_REASON, reason);
    ...
    private class ChatRoomListener implements EMChatRoomChangeListener {
        ...
        public void onRemovedFromChatRoom(int reason, String roomId, String roomName, String participant) {
            if(TextUtils.equals(com.qingkouwei.handyinstruction.DemoHelper.getInstance().getCurrentUser(), participant)) {
                setChatRoomEvent(roomId, EaseEvent.TYPE.CHAT_ROOM);
