...
import com.qingkouwei.handyinstruction.section.chat.adapter.PickAllUserAdapter;
import com.qingkouwei.handyinstruction.section.chat.adapter.PickUserAdapter;
...
public class PickAtUserActivity extends BaseInitActivity implements OnRefreshListener, OnItemClickListener, EaseSidebar.OnTouchEventListener, EaseTitleBar.OnBackPressListener {
    ...
    private void removeSelf(List<EaseUser> data) {
        ...
        while (iterator.hasNext()) {
            ...
            if(TextUtils.equals(user.getUsername(), com.qingkouwei.handyinstruction.DemoHelper.getInstance().getCurrentUser())) {
                iterator.remove();
            }
        }
    }
    ...
    private void checkIfAddHeader() {
        EMGroup group = com.qingkouwei.handyinstruction.DemoHelper.getInstance().getGroupManager().getGroup(mGroupId);
        if(group != null) {
            String owner = group.getOwner();
            if(TextUtils.equals(owner, com.qingkouwei.handyinstruction.DemoHelper.getInstance().getCurrentUser())) {
                AddHeader();
            }
        }
