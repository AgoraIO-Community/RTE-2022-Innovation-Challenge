package com.qingkouwei.handyinstruction.section.chat.fragment;
...
import com.qingkouwei.handyinstruction.section.chat.activity.ForwardMessageActivity;
import com.qingkouwei.handyinstruction.section.chat.activity.PickAtUserActivity;
import com.qingkouwei.handyinstruction.section.chat.activity.SelectUserCardActivity;
...
import com.qingkouwei.handyinstruction.section.dialog.DemoListDialogFragment;
import com.qingkouwei.handyinstruction.section.dialog.FullEditDialogFragment;
import com.qingkouwei.handyinstruction.section.dialog.LabelEditDialogFragment;
...
public class ChatFragment extends EaseChatFragment implements OnRecallMessageResultListener {
    ...
    public void initView() {
        ...
        chatLayout.setTargetLanguageCode(com.qingkouwei.handyinstruction.DemoHelper.getInstance().getModel().getTargetLanguage());
    }
...
//        chatLayout.setReportYourSelf(false);
    ...
    private void resetChatExtendMenu() {
        ...
        if(chatType == EaseConstant.CHATTYPE_GROUP && EMClient.getInstance().getOptions().getRequireAck()) {
            EMGroup group = com.qingkouwei.handyinstruction.DemoHelper.getInstance().getGroupManager().getGroup(conversationId);
    ...
    public void initData() {
        ...
        chatLayout.turnOnTypingMonitor(com.qingkouwei.handyinstruction.DemoHelper.getInstance().getModel().isShowMsgTyping());
                    ...
                    public void OnItemClick(View view, int position) {
                        switch (position) {
                            case 0 :
                                ...
                                DemoLog.e(TAG, "aaaa:" + conversationId + "; username = " + DemoHelper.getInstance().getCurrentUser());
                                break;
                            case 1 :
    ...
    public void onUserAvatarClick(String username) {
        if(!TextUtils.equals(username, com.qingkouwei.handyinstruction.DemoHelper.getInstance().getCurrentUser())) {
            EaseUser user = com.qingkouwei.handyinstruction.DemoHelper.getInstance().getUserInfo(username);
            if(user == null){
                    user = new EaseUser(username);
                }
                boolean isFriend =  com.qingkouwei.handyinstruction.DemoHelper.getInstance().getModel().isContact(username);
                if(isFriend){
                    user.setContact(0);
                }else{
                    user.setContact(3);
    ...
    private void saveUnSendMsg(String content) {
        com.qingkouwei.handyinstruction.DemoHelper.getInstance().getModel().saveUnSendMsg(conversationId, content);
    }

    private String getUnSendMsg() {
        return com.qingkouwei.handyinstruction.DemoHelper.getInstance().getModel().getUnSendMsg(conversationId);
    }
                                ...
                                EMClient.getInstance().chatManager().asyncReportMessage(message.getMsgId(), label, reason, new EMCallBack() {
                                    ...
                                    public void onError(int code, String error) {
                                        ...
                                        runOnUiThread(new Runnable() {
                                            @Override
                                            public void run() {
                                                Toast.makeText(getContext(),"举报失败： code: " + code + " desc: " + error,
