...
from GUI.student_register import Ui_Register_MainWindow
...
class RegisterWindow(QMainWindow,Ui_Register_MainWindow):
    ...
    def __init__(self,leancloud_cfg = "config.yaml"):
        ...
        self.STU_CHAT_TABLE = cfg["leancloud_chatroom"]  # 房间表
    ...
    def register(self):
        ...
        if(stu != None):
            ...
            Stu = leancloud.Object.extend(self.STU_LOGIN_TABLE)
            stu = Stu()
            stu.set("name",stuName)
            sha1_pwd = shaEncode(password)  #使用sha1对密码进行加密
            stu.set("password",sha1_pwd)
            stu.save()
