...
function SendPaperModalWrap(props: Props, ref: React.Ref<unknown> | undefined) {
  ...
  const [givePaperSource, setGivePaperSource] = useState<any[]>();
  ...
  useImperativeHandle(ref, () => ({
    show: async (type: SendPapersType) => {
      ...
      if (type === SendPapersType.已下发列表) {
        setVisible(true);
        if (props.onGetGivePaper) {
          const data = await props.onGetGivePaper();
          setGivePaperSource(data);
        }
      }
    },
            ...
            <Space>
              <a
                onClick={async (e) => {
                  ...
                  if (row['__type__'] === 'CUSTOM') {
                    await props!?.onUnGivePaper({
                      type: SendPapersType.自定义下发,
                      id,
                      customName: row.name,
                      customUrl: row.url,
                      customType: row.type,
  ...
  const handlSelectSendPaper = async () => {
    ...
    if (type === SendPapersType.指定下发) {
      req.id = lodash.get(selectedRowKeys, '[0].id', undefined);
      if (!req.id) {
        Modal.warn({
          title: '请先选择一份试卷',
        });
    ...
    if (type === SendPapersType.自定义下发) {
      const { customUrl, customType, customName } = form.getFieldsValue();
      if (!customUrl) {
        Modal.warn({
          title: '请先填写自定义URL',
        });
      ...
      req.customType = customType || 'PDF';
    }
    try {
      await props!?.onSelectSendPaper(req);
      setVisible(false);
    } catch (err) {}
  };
          ...
          <div className={styles.mainBox}>
            ...
            >
              <div className={styles.scrollbarsWrap}>
                ...
                }
                {
                  ...
                  <Scrollbars>
                    ...
                    <Form.Item
                        name="customName"
                        required
                        rules={[{ type: 'string', required: true, message: '请输入试卷名' }]}
                      >
                      ...
                      <Form.Item
                        name="customUrl"
                        required
                        help={"自定义下发只能下发网页url和PDF的url"}
                        rules={[{ type: 'url', required: true, message: '请输入url(例: https://xxxx.xx.com)' }]}
                      >
                        <Input addonBefore={(
                          <Form.Item noStyle
                            name="customType"
                          ...
                          placeholder="https://xxx.xxx.com/test.pdf"
                        />
                      </Form.Item>
                    </div>
                  </Scrollbars>
                ...
                {
                  type === SendPapersType.已下发列表 ?
                  <Scrollbars>
                    <Table
                      size="small"
                      bordered
                      rowKey={(i: any) => i!?.id}
                      dataSource={givePaperSource||[]}
                      columns={givePaperColumns}
                      pagination={false}
                    />
                  </Scrollbars>
                  : undefined
              ...
              <div className={styles.btnsWrap}>
                {
                  type !== SendPapersType.已下发列表 ? <Form.Item noStyle>
                    <Space>
                      <Button
                        onClick={() => {
                          setVisible(false);
