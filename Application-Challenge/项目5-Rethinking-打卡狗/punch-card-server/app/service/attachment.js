...
class AttachmentService extends Service {
  ...
  async index(params) {
    ...
    // await this.ctx.model.Attachment.sync({ force: true });

    return await this.ctx.model.Attachment.findAll();
  ...
  async findByIdAndUpdate(id, params) {
    const user = await this.ctx.model.Attachment.findByPk(id)
    if (!user) {
      this.ctx.throw(404, `附件不存在 ${id}`);
    }
  ...
  async upload(stream, filename, extname) {
    ...
    attachment.updateTime = timeStamp;
    attachment.id = timeStamp;
  ...
  async handleStream(stream, params) {
    ...
    console.log("uploadDir: " + this.config.uploadDir);
    console.log(params);
