...
@interface DCTabViewController ()<UITabBarControllerDelegate,EMChatManagerDelegate, EaseIMKitManagerDelegate>
@property (nonatomic) BOOL isViewAppear;
@property (nonatomic,strong)DCTabBar    *tabBar;
...
+(instancetype)sharedMainTabBarController{
    static DCTabViewController *tabBarController = nil;
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.delegate = self;
    self.tabBar = [[DCTabBar alloc] init];
...
//    pond.tabBarItem.selectedImage = [UIImage imageNamed:@"理财-1"];
    //不设置图片,先占位
    DCBlockViewController *release = [[DCBlockViewController alloc] init];
