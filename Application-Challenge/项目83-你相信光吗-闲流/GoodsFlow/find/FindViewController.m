...
//  Goods FLow
//
//  Created by zhangyu on 2022/7/22.
//
...
- (void)initSubViews{
    ...
    UIView * topShadowView = [[UIView alloc] initWithFrame:(CGRectMake(0, 10, KScreenWidth, 174 * KScreenRatio))];
    ...
    UIView * backTFView = [[UIView alloc] initWithFrame:(CGRectMake(20, 70 * KScreenRatio+10, KScreenWidth - 40, 35))];
    backTFView.backgroundColor = [UIColor colorWithWhite:239/255.0 alpha:1];
    backTFView.layer.cornerRadius = 17.5;
    backTFView.layer.masksToBounds = YES;
    [topShadowView addSubview:backTFView];
    UITextField * searchTf = [[UITextField alloc] initWithFrame:(CGRectMake(20, 0, KScreenWidth - 40, 35))];
    ...
    UIView * chooseBackView = [[UIView alloc] initWithFrame:(CGRectMake(0, 204 * KScreenRatio + 10, KScreenWidth, hei))];
...
//    menu1.rotateIcon      = [UIImage imageNamed:@"arrowIcon3"];
    ...
    UITableView * tableView = [[UITableView alloc] initWithFrame:(CGRectMake(0, 274 * KScreenRatio + 10, KScreenWidth, KScreenHeight - (274 * KScreenRatio) - 59 - EMVIEWTOPMARGIN)) style:(UITableViewStylePlain)];
...
- (void)searchPeople{
    ...
    for (int i = 0; i < self.totalArr.count; i++) {
        DCMessageModel * model = self.totalArr[i];
    ...
    for (int i = 0; i < dateArr.count; i++) {
        DCMessageModel * model = dateArr[i];
    ...
    for (int i = 0; i < secondArr.count; i++) {
        DCMessageModel * model = secondArr[i];
        NSString * gender = model.pcategory;
        if ([self.searchCondition isEqualToString:gender] || self.searchCondition.length == 0 || [self.searchCondition containsString:@"取货方式"]) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    DCMessageModel * model = self.totalArr[indexPath.row];
