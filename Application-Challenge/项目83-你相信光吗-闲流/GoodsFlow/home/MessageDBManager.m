...
-(void) openDB
{
    ...
    NSString *dbfile = [path stringByAppendingFormat:@"/goodslist.db"];
...
{
    NSString *sql_stmt =
    @"create table if not exists goodstable (goodsid text PRIMARY KEY     NOT NULL, title text, des text, address text,phoneNum text,imgName text,timestamp integer,name text,category text,userid text)";
...
-(void) saveUserInfos
{
    if([self.database isOpen] && self.usersInfoArray.count > 0) {
        NSString* stmt = @"INSERT into goodstable (goodsid,title,des,address,phoneNum,imgName,timestamp,name,category,userid) values ";
//        BOOL bFirst = YES;
        [self.lock lock];
        ...
        FMResultSet *res = [self.database executeQuery:@"SELECT * FROM goodstable"];
        //获取数据库中最大的ID
        while ([res next]) {
            if ([maxID integerValue] < [[res stringForColumn:@"goodsid"] integerValue]) {
                maxID = @([[res stringForColumn:@"goodsid"] integerValue] ) ;
            }
        ...
        DCMessageModel* userInfo = self.usersInfoArray.firstObject;
        stmt = [stmt stringByAppendingFormat:@"('%@','%@','%@','%@','%@','%@',%ld,'%@','%@','%@')",format([NSString  stringWithFormat:@"%@",maxID]),format(userInfo.title),format(userInfo.des),format(userInfo.address),userInfo.phoneNum,format(userInfo.imgName),ts,userInfo.pName,userInfo.pcategory,userInfo.userid];
//        for(DCMessageModel* userInfo in self.usersInfoArray) {
//            if(bFirst) {
//                bFirst = NO;
...
-(void)deleteOutDateData
{
    if([self.database isOpen]) {
        ...
        NSString* stmt = [NSString stringWithFormat:@"delete from goodstable where timestamp < %ld",ts];
        if ([self.database executeUpdate:stmt]) {
...
-(NSArray<DCMessageModel*>*) loadMessages
{
    ...
    if([self.database isOpen]) {
        NSString* stmt = @"select goodsid,title,des,address,phoneNum,imgName,timestamp,name,category,userid from goodstable ";
        FMResultSet *result = [self.database executeQuery:stmt];
        while ([result next])
        {
            DCMessageModel* userInfo = [[DCMessageModel alloc] init];
            userInfo.goodsid = [result stringForColumnIndex:0];
            ...
            userInfo.pcategory = [result stringForColumnIndex:8];
