...
@property(nonatomic,weak)UIButton *quBtn;

@property(nonatomic,weak)UIButton *laiBtn;
...
- (void)initSubviews{
    ...
    UILabel * cateoryLB = [[UILabel alloc] init];
    cateoryLB.x = lbx;
    cateoryLB.y = 475;
    cateoryLB.width = KScreenRatio * 170;
    cateoryLB.height = 25;
    cateoryLB.text = @"取货方式";
    cateoryLB.textColor = KFontColor;
    [self.view addSubview:cateoryLB];
    
    UILabel *quLB = [[UILabel alloc] init];
    quLB.text = @"上门自取";
    quLB.textColor = KFontColor;
    quLB.x = tfx;
    quLB.y = 475;
    quLB.width = 75;
    quLB.height = 25;
    [self.view addSubview:quLB];
    
    UIButton * quBtn = [UIButton buttonWithType:(UIButtonTypeCustom)];
    quBtn.x = tfx + 80;
    quBtn.y = 475;
    quBtn.width = 25;
    quBtn.height = 25;
    [quBtn setImage:[UIImage imageNamed:@"categoryNormal"] forState:(UIControlStateNormal)];
    [quBtn setImage:[UIImage imageNamed:@"categorySelected"] forState:(UIControlStateSelected)];
    [quBtn addTarget:self action:@selector(chooseCateroy:) forControlEvents:(UIControlEventTouchUpInside)];
    self.quBtn = quBtn;
    [self.view addSubview:quBtn];
    self.quBtn.selected = true;
   
    UILabel * laiLB = [[UILabel alloc] init];
    laiLB.text = @"送货上门";
    laiLB.textColor = KFontColor;
    laiLB.x = tfx + 105;
    laiLB.y = 475;
    laiLB.width = 75;
    laiLB.height = 25;
    [self.view addSubview:laiLB];
    
    UIButton * laiBtn = [UIButton buttonWithType:(UIButtonTypeCustom)];
    laiBtn.x = tfx + 180;
    laiBtn.y = 475;
    laiBtn.width = 25;
    laiBtn.height = 25;
    [laiBtn setImage:[UIImage imageNamed:@"categoryNormal"] forState:(UIControlStateNormal)];
    [laiBtn setImage:[UIImage imageNamed:@"categorySelected"] forState:(UIControlStateSelected)];
    [laiBtn addTarget:self action:@selector(chooseCateroy:) forControlEvents:(UIControlEventTouchUpInside)];
    self.laiBtn = laiBtn;
    [self.view addSubview:laiBtn];
...
- (void)chooseCateroy:(UIButton *)sender{
    if (sender == self.quBtn) {
        sender.selected = true;
        self.laiBtn.selected = false;
    }else{
        sender.selected = true;
        self.quBtn.selected = false;
    }
}
...
- (void)saveLocalMessage:(NSString *)mp4Address{
    ...
    if (self.quBtn.selected == true) {
        model.pcategory = @"送货上门";
    }else{
        model.pcategory = @"上门自提";
    }
